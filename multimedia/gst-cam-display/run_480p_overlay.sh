...
#   scales/crops to 640x480 RGB, and renders as a 640x480 overlay
#   at position (0,0) on the HDMI output using the mpfs-dpsub driver.
#
# Prerequisites:
...
# Find the overlay plane ID for the mpfs-dpsub driver.
# In modetest output, the "type" property has value: 0 for Overlay
# (enums: Overlay=0 Primary=1 Cursor=2).
OVERLAY_PLANE_ID=$(modetest -M mpfs-dpsub -p 2>/dev/null | \
    awk '
        /^Planes:/ { in_planes=1; next }
        /^Frame buffers:/ { in_planes=0 }
        in_planes && /^[0-9]/ { plane_id=$1 }
        in_planes && /[0-9]+ type:/ { in_type=1; next }
        in_type && /value:/ { if ($2 == 0) { print plane_id; exit }; in_type=0 }
    ')

if [ -z "$OVERLAY_PLANE_ID" ]; then
    echo "Error: Could not find overlay plane for mpfs-dpsub driver" >&2
    exit 1
fi

echo "Using overlay plane ID: $OVERLAY_PLANE_ID"
...
gst-launch-1.0 -v v4l2src device=/dev/video0  ! video/x-raw, width=640, height=480 ! videoconvert ! kmssink driver-name=mpfs-dpsub plane-id=$OVERLAY_PLANE_ID skip-vsync=true plane-properties="s,alpha=254" render-rectangle="<0,0,640,480>"
